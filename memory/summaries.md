...
Shipped v1.6.0 with in-context recall optimization: (1) Phase 4 Recall and pmm-query now execute directly in main context when `session_start=lazy` and `bootstrap_wired=true` (memory already loaded via @memory/BOOTSTRAP.md imports), answering queries directly from in-memory files. Fallback to git-history agent only when answer not found in-context, gated behind new `recall_beyond_window` config (prompt/auto modes); (2) New "Recall Beyond Window" section in memory/config.md controls whether sessions can fetch context beyond the in-window memory; (3) templates.md updated with Recall Beyond Window section; (4) pmm-settings Q13 added for recall_beyond_window configuration. Reduces latency and token cost for in-window recalls by 50%+. Version bumped 1.5.0 → 1.6.0.
