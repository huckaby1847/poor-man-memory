...
**2026-03-19** — v1.9.0 shipped via PR #34: branch v1.9.0-tiered-loading created by leith-dev, reviewed and approved by raffi-ismail with comment "Correct diagnosis and clean fix. Tier 1/Tier 2 split is the right model. Moving CLAUDE.md to merge in version.json is overdue. Bootstrap Check now wires full ## Memory section. Edit/Write(memory/*) removes last friction point. LGTM." Merged (squash), branch deleted. Root cause diagnosis: @-imports don't recurse in Claude Code; solution: (1) moved all 12 Tier 1 files as direct @-imports in CLAUDE.md under ## Memory section, (2) kept 4 Tier 2 files (graph.md, vectors.md, taxonomies.md, assets.md) as on-demand haiku agent reads. Files updated: CLAUDE.md, BOOTSTRAP.md, .claude/settings.json, references/templates.md, SKILL.md. pmm/version.json moved CLAUDE.md from system to merge, bumped to v1.9.0. Review feedback: lazy session-start bug present since v1.5.0; @-imports never recurse, assumption never verified. [user:raffi] [agent:leith]

**2026-03-19** — v1.9.0 README published: settings.json code snippet updated with Edit/Write(memory/*) arrays; CLAUDE.md step 3 shows full Tier 1 imports block listing all 13 files; new Memory Files table with Tier column (Tier 1: 13 in-context, Tier 2: 4 on-demand). GitHub release v1.9.0 published at https://github.com/NominexHQ/poor-man-memory/releases/tag/v1.9.0 by raffi-ismail. [system:process]
